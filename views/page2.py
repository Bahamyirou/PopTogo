...
with st.container():
    try:
        ...
        # Calculate Togo bounds
        bounds = gdf.bounds
        min_lat, min_lon = bounds.miny.min(), bounds.minx.min()
        max_lat, max_lon = bounds.maxy.max(), bounds.maxx.max()
        
        # Calculate center and zoom to fit Togo
        center_lat = (min_lat + max_lat) / 2
        center_lon = (min_lon + max_lon) / 2
        
        # Create base map focused on Togo
        m = folium.Map(
            location=[center_lat, center_lon],
            zoom_start=8,  # Higher zoom for country-level view
            min_zoom=6,    # Prevent zooming out too far
            max_zoom=12,   # Allow detailed zoom
            tiles='OpenStreetMap',
            max_bounds=True  # Restrict panning outside bounds
        )
        
        # Fit map to Togo boundaries with padding
        padding = 0.05  # Add small padding around Togo
        ...
        top_5 = gdf.nlargest(5, 'PopCount')[['NAME', 'PopCount']]
